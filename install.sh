...
read -p "Where should I install Riven TUI? [Default: $DEFAULT_DIR]: " INSTALL_DIR
INSTALL_DIR=${INSTALL_DIR:-$DEFAULT_DIR}

if [ -d "$INSTALL_DIR" ]; then
    echo -e "${YELLOW}Directory $INSTALL_DIR already exists.${NC}"
    read -p "Should I overwrite it? (y/N): " OVERWRITE
...
read -p "Riven API Key: " RIVEN_API_KEY
read -p "TMDB Bearer Token: " TMDB_TOKEN
read -p "Riven Backend Protocol [http]: " RIVEN_PROTO
RIVEN_PROTO=${RIVEN_PROTO:-http}
read -p "Riven Backend Host [localhost]: " RIVEN_HOST
RIVEN_HOST=${RIVEN_HOST:-localhost}
read -p "Riven Backend Port [8080]: " RIVEN_PORT
...
if ! command -v chafa &> /dev/null; then
    ...
    read -p "Would you like to try installing chafa? (Requires sudo) [Y/n]: " INSTALL_CHAFA
...
read -p "Would you like to add a 'riven-tui' alias to your shell profile? [y/N]: " ADD_ALIAS
